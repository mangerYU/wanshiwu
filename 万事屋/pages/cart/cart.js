...
//   },
  ...
  btn_submit_order: function (e) {
    ...
    if(totalPrice==0){
      ...
      let pro_name = this.data.list[index].pro_name;
      let num = this.data.list[index].num;
      let price = this.data.list[index].price;
      list[index].selected = !list[index].selected;
   console.log("spName" + title + "type" + pro_name + "price" + price + "num" + num);  
      wx.navigateTo({
        url: '/pages/orders/orders?spName=' + title + "&type=" + pro_name + "&price=" + price + "&num=" + num,
      })
